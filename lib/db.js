...
function value_to_string(value) {
    if (typeof value == "boolean")
        return colors.yellow(value ? "true" : "false");
    if (typeof value == "number")
        return colors.green(value.toString());
    if (typeof value == "string") {
        return colors.blue("\"".concat(value.replaceAll('"', '\\"'), "\""));
    }
}
var Validation;
(function (Validation) {
    ...
    schema: {
        rows: [
            //Note that we aren't specifying unique: false here. Rows default to not being unique
            {
                name: "username",
                type: exports.STRING,
                validation: function (value) { return value.length <= 40; },
            },
        ],
    },
});
...
db.insert({
    table: "users",
    record: {
        username: "This is an unnecessarily, egregiously, shockingly, utterly, DEVASTATINGLY long. Seriously, you really can't let this happen.",
    },
});
// console.time();
