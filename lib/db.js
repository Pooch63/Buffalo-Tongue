...
var Table = /** @class */ (function () {
  ...
  Table.prototype.insert = function (record) {
    if (!this.schema.validate(record)) {
      throw new Error(
        'Record inserted into table "'.concat(
          this.name,
    ...
    if (unique != null) {
      ...
      if (this.data[String(unique)] != null) {
        throw new Error(
          'Unique key "'.concat(unique, '" has already been inserted.')
        );
      }
    }
        ...
        if (!row.validation(record[row.name])) {
          throw new Error(
            'Custom validation function declared that column "'
              .concat(row.name, '" was invalid (value was ')
              .concat((0, logs_1.value_to_string)(record[row.name]), ")")
          );
        }
      }
    }
...
var Database = /** @class */ (function () {
  ...
  Database.prototype.get_table = function (table) {
    if (this.tables[table] == null) {
      throw new Errors.Nonexistent_Value(
        'Table "'.concat(table, '" does not exist.')
      );
    }
  ...
  Database.prototype.create_table = function (name, schema) {
    ...
    if (this.tables[name] != null) {
      throw new Errors.Name_Already_Exists(
        'Table name "'.concat(name, '" already exists.')
      );
    }
    var parsed_schema = new schema_validation_1.Schema.Table({
  ...
  Database.prototype.select_distinct = function (
    table_name,
    columns,
    condition
  ) {
    ...
    for (var _i = 0, col_arr_1 = col_arr; _i < col_arr_1.length; _i++) {
      ...
      if (!table.schema.col_exists(col)) {
        throw new Errors.Nonexistent_Column(
          'Select distinct requested distinct, nonexistent column "'.concat(
            col,
            '"'
          )
        );
  ...
  Database.prototype.update = function (table_name, update, condition) {
    ...
    for (var _i = 0, _a = table.schema.columns; _i < _a.length; _i++) {
      ...
      if (
        ...
        !schema_validation_1.Schema.validate_type({
          value: update[col.name],
          type: col.type,
        })
      ) {
        throw new Errors.Bad_Type(
          'Invalid value provided for column in update statement -- "'
            .concat(col.name, '" is not of valid type (valid type is of ')
            .concat(
              (0, logs_1.value_type_as_string)(col.type),
              ", instead got value "
            )
            .concat((0, logs_1.value_to_string)(update[col.name]), " as ")
            .concat((0, logs_1.value_type_as_string)(update[col.name]), ")")
        );
      }
    }
    table.each(function (row) {
